...
public class TriggeredBehaviour_TogglePatroller : TriggeredBehaviour
{
	public enum SetType
	{
		Toggle,
	...
	public PlatformGenerator_Patrol_Direct patroller;
	public SetType setType = SetType.Toggle;
	...
	private void SetPatroller ()
	{
		if (patroller == null)
			return;

		//TODO: Baby steps to becoming a tangly mess
		if (setType == SetType.Toggle)
		{
			if (patroller.isPatrolling)
				patroller.RequestPatrolEnd ();
			else
				patroller.RequestPatrolStart ();
		}
		else if (setType == SetType.Activate)
		{
			patroller.RequestPatrolStart ();
		}
		else if (setType == SetType.Deactivate)
		{
			patroller.RequestPatrolEnd ();
		}
	}
